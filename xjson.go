...
func Get[T any](db map[string]any, xs []string) (T, error) {
	...
	for n, x := range xs {
		...
		if !ok {
			return tnil, fmt.Errorf(
				"%w: %s", BadPathError, PathString(xs[:n+1]),
			)
		}
		...
		if n == len(xs)-1 {
			...
			if !ok {
				return tnil, fmt.Errorf(
					"%w: '%s'; got '%T', expected '%T'", BadTypeError,
	...
	return tnil, fmt.Errorf(
		"%w: %s", BadPathError, PathString(xs),
	)
}
...
func WriteIndentFile(fn string, db map[string]any) error {
	xs, err := json.MarshalIndent(db, "", "\t")
	if err != nil {
		return fmt.Errorf("marshaling: %w", err)
	}
